...
const VarietiesSection = () => {
  ...
  const getColor = (name: string) =>
    colorMap[name] || 'from-gray-900/60 to-gray-600/60';
  ...
  const handleViewVariety = (varietyId: string) => {
    router.push(`/variety/${varietyId}`);
  };

  const itemsToShow = 1;
  const maxIndex = Math.max(0, varieties.length - itemsToShow);
        ...
        <div className="hidden md:grid md:grid-cols-4 gap-6">
          {varieties.map((variety) => (
            <div
              ...
              className="group relative h-[400px] rounded-lg overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-500"
            >
              <img
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-b ${getColor(
                  variety.name
                )} opacity-40 group-hover:opacity-60 transition-opacity duration-300`}
              />

              <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
                <h3 className="font-antique text-4xl font-bold text-white tracking-wide text-center">
                  {variety.name}
                </h3>

                <button className="mt-6 px-6 py-2 border-2 border-white text-white font-semibold rounded-md opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 hover:bg-white hover:text-gray-800">
                  View Products
                </button>
              </div>
            </div>
            ...
            {varieties.map((variety) => (
              <div key={variety._id} className="min-w-full px-2">
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
                    <h3 className="text-4xl font-bold text-white tracking-wide text-center">
                      {variety.name}
                    </h3>
