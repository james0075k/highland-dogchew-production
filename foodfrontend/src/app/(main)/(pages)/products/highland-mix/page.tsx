...
export default function HighlandMixPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${API_BASE}/products?type=highland-mix`);
      ...
      <section className="pt-[90px] md:pt-[120px] pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <nav className="flex items-center gap-1.5 text-[12px] text-gray-400 dark:text-gray-500 mb-8 font-medium">
            ...
            <ChevronRight className="w-3.5 h-3.5 flex-shrink-0" />
            <span className="text-[#2E1F14] dark:text-[#c8b6a6]">Highland Mix Chews</span>
          </nav>

          {/* Page header */}
          <div className="mb-8 md:mb-10">
            <span className="text-[11px] font-bold tracking-[0.25em] uppercase text-amber-600 dark:text-amber-500 block mb-2">Premium Blend</span>
            ...
            <p className="text-[#7A5C4F] dark:text-[#c8b6a6] text-sm md:text-base max-w-2xl leading-relaxed">
              A premium blend of authentic Himalayan ingredients crafted for a unique, enriching chewing experience your dog will love.
            </p>
          </div>
          ...
          {!loading && !error && sorted.length === 0 && (
            <div className="py-20 text-center">
              <p className="text-gray-500 dark:text-gray-400 text-sm">No highland mix chews found.</p>
            </div>
          )}
