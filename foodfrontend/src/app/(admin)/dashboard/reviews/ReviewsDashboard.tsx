...
export default function ReviewsDashboard() {
  ...
  // Arriving from a product's "Manage reviews" link pre-fills the search so
  // the admin lands directly on that product's reviews instead of the full list.
  const [searchTerm, setSearchTerm] = useState(() => searchParams?.get('product') || '');
  const [deleteTarget, setDeleteTarget] = useState<Review | null>(null);
  // Approve/reject/delete used to update the row optimistically without
  // checking whether the request actually succeeded — an expired session or a
  // dropped request left the review still pending in the database while the
  // dashboard showed it as handled, so the product's rating never moved. Track
  // in-flight actions and surface failures instead of assuming success.
  ...
  const updateStatus = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.message || `Server rejected the request (${res.status})`);
      }
      // Only reflect the change once the server has actually applied it — this
      // is also what triggers the product's rating/review count to resync.
      setReviews(prev => prev.map(r => r._id === id ? { ...r, status } : r));
    } catch (err) {
      setActionError(
        `Couldn't ${ACTION_LABEL[status]} this review: ${err instanceof Error ? err.message : 'unknown error'}. Nothing changed — try again.`
      );
    } finally {
      setActingId(null);
    }
  ...
  const confirmDelete = async () => {
    ...
    setActionError(null);
    try {
      const res = await fetch(`${API}/reviews/${target._id}`, {
        method: 'DELETE',
        headers: authHeader(),
      });
      const data = await res.json();
      ...
      setActionError(`Couldn't delete this review: ${err instanceof Error ? err.message : 'unknown error'}. It hasn't been removed — try again.`);
      setDeleteTarget(null);
    } finally {
      setActingId(null);
    }
            ...
            return (
              <section key={key} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <ul className="divide-y divide-gray-50">
                  {items.map(review => {
                    const isActing = actingId === review._id;
                    return (
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {review.status !== 'approved' && (
                          <button
                            ...
                            disabled={isActing}
                            title="Approve"
                            className="p-2 rounded-lg border border-emerald-200 bg-white text-emerald-600 hover:bg-emerald-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            <FiCheck size={13} />
                          </button>
                        )}
                        {review.status !== 'rejected' && (
                          <button
                            ...
                            disabled={isActing}
                            title="Reject"
                            className="p-2 rounded-lg border border-amber-200 bg-white text-amber-600 hover:bg-amber-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            <FiX size={13} />
                          </button>
                        )}
                        <button
                          ...
                          disabled={isActing}
                          title="Delete"
                          className="p-2 rounded-lg border border-red-200 bg-white text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                          <FiTrash2 size={13} />
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6">
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button onClick={confirmDelete} disabled={actingId === deleteTarget._id} className="flex-1 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold text-sm transition disabled:opacity-50">
                {actingId === deleteTarget._id ? 'Deleting…' : 'Delete'}
              </button>
            </div>
          </div>
        </div>
